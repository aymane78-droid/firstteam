  ...
  return (
    <section className="r-sec-xl" style={{ padding: "120px 40px", background: "#0A0A0A", borderTop: "1px solid #1a1a1a" }}>
      <div className="r-hub-inner" style={{ maxWidth: 1200, margin: "0 auto" }}>
        ...
        <div className="recent-videos-grid" style={{ display: "grid", gridTemplateColumns: "repeat(2, 1fr)", gap: 28, marginBottom: 48 }}>
          {dynamicVideos
      ...
      {/* Social strip */}
      <div className="social-strip" style={{ marginTop: 48, display: "flex", gap: 16, flexWrap: "wrap" as const }}>
          {[
            ...
            >
              ...
              <span className="social-btn-label" style={{ ...ANTON, fontSize: 18, letterSpacing: 0.5, marginBottom: 4 }}>{s.label}</span>
              <span className="social-btn-sub" style={{ ...MANROPE(600), fontSize: 12, opacity: 0.8 }}>{s.sub}</span>
            </a>
          ))}
      </div>
      {modalVideoId && <VideoModal videoId={modalVideoId} onClose={() => setModalVideoId(null)} />}
...
// ── 9. À PROPOS — composant partagé avec la page L'Équipe ────────────
